...
const mockQdrantClient = {
	getCollections: jest.fn<() => Promise<any>>().mockImplementation(async () => {
		// Added type
		return {
			collections: [],
		}
	}),
	getCollection: jest.fn<() => Promise<any>>(), // Added type
	createCollection: jest.fn<() => Promise<any>>(), // Added type
	upsert: jest.fn<() => Promise<any>>(), // Added type
	...
	delete: jest.fn<() => Promise<any>>(), // Added type
}
...
const mockInitialize = jest.fn<() => Promise<boolean>>().mockResolvedValue(true) // Added type
const mockUpsertPoints = jest.fn<(points: any[]) => Promise<void>>().mockImplementation(async (points) => {}) // Added type
// Added type
const mockSearch = jest
	.fn<(queryVector: number[], limit?: number) => Promise<QdrantSearchResult[]>>()
	.mockImplementation(async (queryVector, limit = 10) => {
		return [
			{
				id: "point1",
				score: 0.9,
...
const mockDeletePointsByFilePath = jest.fn<(filePath: string) => Promise<void>>() // Added type
const mockClearCollection = jest.fn<() => Promise<void>>() // Added type
